...
*/
CREATE TABLE train_fencang_window1 AS
SELECT  1 AS watch,a.store_code,a.item_id,label,
...
FROM(
/*标签*/
SELECT store_code,item_id,SUM(qty_alipay_njhs)AS label
FROM (
SELECT a.* FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=0)a GROUP BY store_code,item_id
)a LEFT OUTER JOIN(
/*最近1天*/
SELECT store_code,item_id,AVG(pv_ipv)AS pv_ipv_1,AVG(pv_uv) AS pv_uv_1,AVG(cart_ipv)AS cart_ipv_1,AVG(cart_uv)AS cart_uv_1,
...
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a WHERE days=1)a GROUP BY store_code,item_id
)b ON(a.item_id=b.item_id AND a.store_code=b.store_code) LEFT OUTER JOIN(
/*最近2天*/
SELECT store_code,item_id,AVG(pv_ipv)AS pv_ipv_2,AVG(pv_uv) AS pv_uv_2,AVG(cart_ipv)AS cart_ipv_2,AVG(cart_uv)AS cart_uv_2,
...
SELECT a.*,DATEDIFF('2015/12/14',DATE)AS days FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a WHERE days<=2)a GROUP BY store_code,item_id
)c ON(a.item_id=c.item_id AND a.store_code=c.store_code) LEFT OUTER JOIN(
/*最近3天*/
SELECT store_code,item_id,AVG(pv_ipv)AS pv_ipv_3,AVG(pv_uv) AS pv_uv_3,AVG(cart_ipv)AS cart_ipv_3,AVG(cart_uv)AS cart_uv_3,
...
SELECT a.*,DATEDIFF('2015/12/14',DATE)AS days FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a WHERE days<=3)a GROUP BY store_code,item_id
)d ON(a.item_id=d.item_id AND a.store_code=d.store_code) LEFT OUTER JOIN(
/*最近5天*/
SELECT store_code,item_id,AVG(pv_ipv)AS pv_ipv_5,AVG(pv_uv) AS pv_uv_5,AVG(cart_ipv)AS cart_ipv_5,AVG(cart_uv)AS cart_uv_5,
...
SELECT a.*,DATEDIFF('2015/12/14',DATE)AS days FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a WHERE days<=5)a GROUP BY store_code,item_id
)e ON(a.item_id=e.item_id AND a.store_code=e.store_code) LEFT OUTER JOIN(
/*最近7天*/
SELECT store_code,item_id,AVG(pv_ipv)AS pv_ipv_7,AVG(pv_uv) AS pv_uv_7,AVG(cart_ipv)AS cart_ipv_7,AVG(cart_uv)AS cart_uv_7,
...
SELECT a.*,DATEDIFF('2015/12/14',DATE)AS days FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a WHERE days<=7)a GROUP BY store_code,item_id
)f ON(a.item_id=f.item_id AND a.store_code=f.store_code) LEFT OUTER JOIN(
/*最近9天*/
SELECT store_code,item_id,AVG(pv_ipv)AS pv_ipv_9,AVG(pv_uv) AS pv_uv_9,AVG(cart_ipv)AS cart_ipv_9,AVG(cart_uv)AS cart_uv_9,
...
SELECT a.*,DATEDIFF('2015/12/14',DATE)AS days FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a WHERE days<=9)a GROUP BY store_code,item_id
)g ON(a.item_id=g.item_id AND a.store_code=g.store_code) LEFT OUTER JOIN(
/*最近11天*/
SELECT store_code,item_id,AVG(pv_ipv)AS pv_ipv_11,AVG(pv_uv) AS pv_uv_11,AVG(cart_ipv)AS cart_ipv_11,AVG(cart_uv)AS cart_uv_11,
...
SELECT a.*,DATEDIFF('2015/12/14',DATE)AS days FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a WHERE days<=11)a GROUP BY store_code,item_id
)h ON(a.item_id=h.item_id AND a.store_code=h.store_code) LEFT OUTER JOIN(
/*最近14天*/
SELECT store_code,item_id,AVG(pv_ipv)AS pv_ipv_14,AVG(pv_uv) AS pv_uv_14,AVG(cart_ipv)AS cart_ipv_14,AVG(cart_uv)AS cart_uv_14,
...
SELECT a.*,DATEDIFF('2015/12/14',DATE)AS days FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a WHERE days<=14)a GROUP BY store_code,item_id
)i ON(a.item_id=i.item_id AND a.store_code=i.store_code)LEFT OUTER JOIN(
/*非聚划算销量最值，标准差*/
SELECT store_code,item_id,MAX(qty_alipay_njhs)AS max_qty_alipay_njhs,MIN(qty_alipay_njhs)AS min_qty_alipay_njhs,STD(qty_alipay_njhs)AS std_qty_alipay_njhs
FROM(
SELECT a.*,DATEDIFF('2015/12/14',DATE)AS days FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1)a GROUP BY store_code,item_id
)j ON (a.item_id=j.item_id AND a.store_code=j.store_code)
LEFT OUTER JOIN(
/*item与其他id对应关系*/
SELECT a.*,
b.cate_id_qty_alipay_njhs_sum,b.cate_id_qty_alipay_njhs_avg,b.cate_id_qty_alipay_njhs_std,
c.cate_level_id_qty_alipay_njhs_sum,c.cate_level_id_qty_alipay_njhs_avg,c.cate_level_id_qty_alipay_njhs_std,
d.brand_id_qty_alipay_njhs_sum,d.brand_id_qty_alipay_njhs_avg,d.brand_id_qty_alipay_njhs_std,
e.supplier_id_qty_alipay_njhs_sum,e.supplier_id_qty_alipay_njhs_avg,e.supplier_id_qty_alipay_njhs_std
FROM(
SELECT store_code,item_id,cate_id,cate_level_id,brand_id,supplier_id
FROM item_store_feature GROUP BY store_code,item_id,cate_id,cate_level_id,brand_id,supplier_id
)a LEFT OUTER JOIN (
/*叶子类目id特征*/
SELECT store_code,cate_id,SUM(qty_alipay_njhs)AS cate_id_qty_alipay_njhs_sum,AVG(qty_alipay_njhs)AS cate_id_qty_alipay_njhs_avg,STD(qty_alipay_njhs)AS cate_id_qty_alipay_njhs_std
FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1 GROUP BY store_code,cate_id
)b ON(a.cate_id=b.cate_id AND a.store_code=b.store_code) LEFT OUTER JOIN(
/*大类目ID特征*/
SELECT store_code,cate_level_id,SUM(qty_alipay_njhs)AS cate_level_id_qty_alipay_njhs_sum,AVG(qty_alipay_njhs)AS cate_level_id_qty_alipay_njhs_avg,STD(qty_alipay_njhs)AS cate_level_id_qty_alipay_njhs_std
FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1 GROUP BY store_code,cate_level_id
)c ON(a.cate_level_id=c.cate_level_id AND a.store_code=c.store_code) LEFT OUTER JOIN(
/*品牌ID特征*/
SELECT store_code,brand_id,SUM(qty_alipay_njhs)AS brand_id_qty_alipay_njhs_sum,AVG(qty_alipay_njhs)AS brand_id_qty_alipay_njhs_avg,STD(qty_alipay_njhs)AS brand_id_qty_alipay_njhs_std
FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1 GROUP BY store_code,brand_id
)d ON(a.brand_id=d.brand_id AND a.store_code=d.store_code) LEFT OUTER JOIN(
/*供应商ID特征*/
SELECT store_code,supplier_id,SUM(qty_alipay_njhs)AS supplier_id_qty_alipay_njhs_sum,AVG(qty_alipay_njhs)AS supplier_id_qty_alipay_njhs_avg,STD(qty_alipay_njhs)AS supplier_id_qty_alipay_njhs_std
FROM(
SELECT a.*,FLOOR((DATEDIFF('2015/12/28',DATE)-1)/14) AS watch FROM(
SELECT * FROM item_store_feature )a)a WHERE watch=1 GROUP BY store_code,supplier_id
)e ON(a.supplier_id=e.supplier_id AND a.store_code=e.store_code) 
)k ON(a.item_id=k.item_id AND a.store_code=k.store_code) 
;
